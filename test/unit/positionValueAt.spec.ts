...
describe('Simple Blueprint (Position Value At)', () => {
  ...
  describe('getPositionValue()', () => {
    beforeAll(async () => {
      positionValue = await blueprint.getPositionValueAt(positionContext, timeContext);
    });

    it('should have the position value in USD', () => {
      expect(positionValue.positionValueUsd).toEqual(2001);
    });
    ...
    describe('position shares', () => {
      ...
      it('should have the position balance', () => {
        expect(positionShareDetails[0]?.positionBalance?.eq(2000)).toBeTruthy();
      });
